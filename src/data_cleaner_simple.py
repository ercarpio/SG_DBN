...
for key in sorted(final_samples.keys()):
    ...
    obs = get_obs(sample_values[len(sample_values) - 1])
    if key[0] == 'compliant':
        if key[2] == 0 and obs == 2:
            curr_sample = dict()
            curr_sample[('P', 0)] = 0
            curr_sample[('R', 0)] = 0
            curr_sample[('A', 0)] = 0
    ...
    elif key[0] == 'noncompliant':
        if key[2] == 0 and obs == 0:
            curr_sample = dict()
            curr_sample[('P', 0)] = 0
            curr_sample[('R', 0)] = 0
            curr_sample[('A', 0)] = 0
...
data.to_csv('../data/simple_qval_data.csv')
